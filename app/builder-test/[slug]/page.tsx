...
export default function BuilderEventPage() {
  ...
  const [eventData, setEventData] = useState<BuilderEvent | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  
  // Load event data from Builder.io
  useEffect(() => {
    const loadEventData = async () => {
      try {
        ...
        const event = data.transformed?.find((e: BuilderEvent) => e.slug === slug)
  ...
  if (error || !eventData) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-xl text-red-600">Error: {error || 'Event not found'}</div>
      </div>
    );
  }
      ...
      <section className="alpha-section">
        <div className="flex flex-col md:flex-row gap-[var(--space-lg)] md:items-start">
          ...
          <div className="flex-1">
            <h2 className="heading-style-h3 mb-[var(--space-sm)]">{eventData.descriptionTitle || eventData.title}</h2>
          ...
          <div className="alpha-card scheme-pink w-full md:w-2/5">
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <span>{new Date(eventData.time).toLocaleTimeString('en-US', {
                hour: 'numeric',
                minute: '2-digit',
                hour12: true
              })}</span>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span>{eventData.locationTag}</span>
            </div>
        ...
        <div id="register" className="alpha-section mt-[var(--space-3xl)]">
          ...
          <div className="alpha-form">
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                <label htmlFor="additionalChildren" className="block mb-1 font-medium">Additional Children</label>
                <Textarea id="additionalChildren" value={additionalChildren} onChange={e => setAdditionalChildren(e.target.value)} placeholder="Please list any additional children and their ages" />
              </div>
              <div>
                <label htmlFor="allergies" className="block mb-1 font-medium">Allergies or Special Needs</label>
                <Textarea id="allergies" value={allergies} onChange={e => setAllergies(e.target.value)} placeholder="Please list any allergies or special needs we should be aware of" />
              </div>
              <div>
                <label htmlFor="howHeard" className="block mb-1 font-medium">How did you hear about us?<span>*</span></label>
                <Input id="howHeard" type="text" value={howHeard} onChange={e => setHowHeard(e.target.value)} required />
              </div>
              <div className="flex items-start gap-2">
                ...
                <label htmlFor="consent" className="text-sm">
                  I consent to receive communications from Alpha School regarding this event and future opportunities.<span>*</span>
                </label>
              </div>
              <Button type="submit">
                Register for Event
