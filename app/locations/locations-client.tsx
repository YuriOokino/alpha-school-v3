...
export default function LocationsClient() {
  ...
  const loadCampuses = async () => {
    try {
      ...
      setCurrentCampusesToShow(current.slice(0, 6));
      setUpcomingCampusesToShow(upcoming.slice(0, 6));
      setShowLoadMoreCurrent(current.length > 6);
      setShowLoadMoreUpcoming(upcoming.length > 6);
    } catch (error) {
  ...
  const loadMoreCurrent = () => {
    ...
    const nextBatch = currentCampuses.slice(currentShown, currentShown + 6);
    setCurrentCampusesToShow([...currentCampusesToShow, ...nextBatch]);
    setShowLoadMoreCurrent(currentShown + 6 < currentCampuses.length);
  };

  const loadMoreUpcoming = () => {
    const upcomingShown = upcomingCampusesToShow.length;
    const nextBatch = upcomingCampuses.slice(upcomingShown, upcomingShown + 6);
    setUpcomingCampusesToShow([...upcomingCampusesToShow, ...nextBatch]);
    setShowLoadMoreUpcoming(upcomingShown + 6 < upcomingCampuses.length);
  };
  ...
  return (
    <main className="min-h-screen bg-[var(--color-bg-muted)]">
      ...
      <div className="alpha-section">
        <div className="alpha-card flex flex-col gap-8 justify-center items-center bg-[var(--color-navy-blue)] mx-auto max-w-[600px] w-full">
          <h3 className="text-[var(--color-sky-blue)] text-center heading-style-h4">Find a campus near you</h3>
      ...
      <div className="alpha-section bg-white">
        <div className="mb-[var(--space-4xl)]">
          <SectionHeading 
            ...
            description="Discover Alpha School campuses across the country where students are already learning 2x faster, building confidence, and loving school again. Each location offers our proven curriculum and personalized learning approach." 
        ...
        <div id="new-locations">
          <SectionHeading 
            description="Alpha School is expanding! New campuses opening this fall will bring our innovative learning model to more communities. Join the waitlist to be among the first families to experience Alpha's transformative education." 
            buttonText="Apply to Alpha School"
            buttonHref="/admission"
       ...
       <div className="alpha-section bg-white">
        ...
        <div className="alpha-card flex flex-col gap-2 mt-[var(--space-3xl)] bg-[var(--color-sky-blue)] text-[var(--color-primary)] max-w-[600px] m-auto">
